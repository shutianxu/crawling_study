...
def Create_table():
    query = """CREATE TABLE IF NOT EXISTS `anjuke_collecter_original_xst` (
...
def etl_mysql(result):
    ...
    cursor = db.cursor()
    result = tuple(result)
    query = "insert into anjuke_collecter_original_xst(housetitle,an_xian,houseencode,housetotleprice,house_model_detail,house_size,house_estate,house_add,house_build_time,house_type,house_orientation,house_floor,house_decorate,house_univalence,down_payment,housedetail,salerphone,Url,SpiderTime) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % result
    cursor.execute(query)
    db.commit()
    cursor.close()
    db.close()
            ...
            if urllist != None:
                for houseurl in urllist:
                    ...
                    try:
                        ...
                        results = None
                        with open(r'F:\WorkSpace\anjuke\anjuke_error_shanghai.txt','a',encoding='utf-8') as f:
                            f.write(houseurl +"\n")
                        print("parse hosue detial infor error!")
                        continue
                    with open(r'F:\WorkSpace\anjuke\anjuke_shanghai_v15.txt','a',encoding='utf-8') as f:
                        f.write(results + '\n')
                    try:
                        line = results.split('\t')
